...
def loss_select(loss, opt, to_optim):
    ...
    """
    if loss=='triplet':
        loss_params  = {'sampling_method':opt.sampling}
...
class ContrastiveLoss(torch.nn.Module):
    ...
    def forward(self, dis_mat, targets, margin):
        #print("dis_mat:",dis_mat.shape)
        #print("targets:",targets.shape)
        n = dis_mat.size(0)
        # Compute similarity matrix
        # dis_mat = compute_distance(inputs)
        loss = list()

        for i in range(n):
            pos_pair_ = torch.masked_select(dis_mat[i], targets==targets[i])

            #  move itself
            #pos_pair_ = torch.masked_select(pos_pair_, pos_pair_ < 1)
            neg_pair_ = torch.masked_select(dis_mat[i], targets!=targets[i])
...
class TripletLoss(torch.nn.Module):
    def __init__(self, sampling_method='random'):
        """
    ...
    def forward(self, batch, labels,margin):
        """
        Args:
        ...
        #Compute triplet loss
        loss             = torch.stack([self.triplet_distance(batch[triplet[0],:],batch[triplet[1],:],batch[triplet[2],:],margin) for triplet in sampled_triplets])
